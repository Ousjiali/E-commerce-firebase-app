...
const GmHrOperationsPendingPage = () => {
  React.useEffect(() => {
    sp.profiles.myProperties.get().then((response) => {
      sp.web.lists
        .getByTitle(`Admin`)
        .items.filter(
          `Title eq '${response.DisplayName}' and Role eq 'General Manager, HR Operations'`
        )
        .get()
        .then((res) => {
          if (res.length === 0) {
            swal(
              "Warning!",
              "You are not General Manager, HR Operations,You are not authorize to access this page",
              "error"
            );
            history.push("/");
          }
  ...
  React.useEffect(() => {
    ...
    sp.web.lists
      ...
      .items.filter(
        `ConfirmationStatus eq 'Pending' and Approvals eq 'GM HR Operations'`
      )
      .get()
      .then((res) => {
        setData(res);
