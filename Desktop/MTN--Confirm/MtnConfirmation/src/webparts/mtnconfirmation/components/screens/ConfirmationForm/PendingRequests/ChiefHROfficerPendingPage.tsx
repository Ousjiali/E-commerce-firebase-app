...
const ChiefHROfficerPendingPage = () => {
  React.useEffect(() => {
    sp.profiles.myProperties.get().then((response) => {
      sp.web.lists
        .getByTitle(`Admin`)
        .items.filter(
          `Title eq '${response.DisplayName}' and Role eq 'Chief Human Resource Officer'`
        )
        .get()
        .then((res) => {
          if (res.length === 0) {
            swal(
              "Warning!",
              "You are not Chief Human Resource Officer,You are not authorize to access this page",
              "error"
            );
            history.push("/");
          }
  ...
  React.useEffect(() => {
    ...
    sp.web.lists
      ...
      .items.filter(
        `ConfirmationStatus eq 'Pending' and Approvals eq 'Chief HR Officer'`
      )
      .get()
      .then((res) => {
        setData(res);
...
export default ChiefHROfficerPendingPage;
