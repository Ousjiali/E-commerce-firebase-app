...
const SeniorManagerESPendingPage = () => {
  React.useEffect(() => {
    sp.profiles.myProperties.get().then((response) => {
      sp.web.lists
        .getByTitle(`Admin`)
        .items.filter(
          `Title eq '${response.DisplayName}' and Role eq 'Senior Manager, Employee Service'`
        )
        .get()
        .then((res) => {
          if (res.length === 0) {
            swal(
              "Warning!",
              "You are not Senior Manager, Employee Service,You are not authorize to access this page",
              "error"
            );
            history.push("/");
          }
  ...
  React.useEffect(() => {
    ...
    sp.web.lists
      ...
      .items.filter(
        `ConfirmationStatus eq 'Pending' and Approvals eq 'Senior Manager Employee Services'`
      )
      .get()
      .then((res) => {
        setData(res);
