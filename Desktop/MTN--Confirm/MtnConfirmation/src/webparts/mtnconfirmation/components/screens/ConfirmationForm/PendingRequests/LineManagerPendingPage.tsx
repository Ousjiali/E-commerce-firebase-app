  ...
  React.useEffect(() => {
    ...
    sp.profiles.myProperties.get().then((response) => {
      sp.web.lists
        ...
        .items.filter(
          `ConfirmationStatus eq 'Pending' and Approvals eq 'Rater Line Manager' and RaterLineManager eq '${response?.Email}' `
        )
        .get()
        .then((res) => {
          if (res.length === 0) {
            swal(
              "Warning!",
              "You are not a Rater Line Manager,You are not authorize to access this page",
              "error"
            );
            history.push("/");
          } else {
