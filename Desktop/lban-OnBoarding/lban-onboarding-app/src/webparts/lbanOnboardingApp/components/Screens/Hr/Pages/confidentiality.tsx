...
export const HRConfidentiality = ({ match }) => {
  ...
  React.useEffect(() => {
    sp.profiles.myProperties.get().then((res) => {
      sp.web.lists
        ...
        .get()
        .then((response) => {
          setID(response[0].StaffName);

          setConfidentiality(response[0].Confidentiality);
        });
    });
  }, []);

  const nextHandler = () => {
    history.push(`/hr/profile-guarantor/${itemID}`);
  };
    ...
    <AppWrapper menu={HRMenu} showBackButton={true}>
      {loading ? (
        ...
        <div className="Displayflex">
          <HRMenuBar confidentiality={"remote"} />
          ...
          <div>
            ...
            >
              <input
                ...
                required={true}
                checked={confidentiality === "completed"}
              />

              <span style={{ marginLeft: "20px" }}>
                {" "}
                I {ID} agrees to hold confidential or proprietary information or
                trade secrets ("confidential information") in trust and
                confidence and agrees that it shall be used only for the
                contemplated purposes, shall not be used for any other purpose,
